...
export class StudentComponent implements OnInit {
  ...
  loadMyStudent(): void {
    ...
    if (studentId) {
      this.studentManagementService.getStudent(+studentId).subscribe({
        next: (data: Student) => {
          this.myStudent = data;
          this.errorMessage = '';
        },
        error: (err: any) => {
          this.errorMessage = 'Failed to fetch your student data.';
        }
      });
    } else {
      this.errorMessage = 'No student ID found in local storage.';
    }
  }
  ...
  addStudent(): void {
    ...
    this.studentManagementService.addStudent(newStudent).subscribe({
      next: (student) => {
        ...
        this.errorMessage = '';
        this.validationMessage = '';
        this.successMessage = 'The student has been added to the system.';
  ...
  submitStudentForm(): void {
    if (this.studentForm.valid) {
      this.studentManagementService.addStudent(this.studentForm.value).subscribe({
        next: () => {
